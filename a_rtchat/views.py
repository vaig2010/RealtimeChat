...
@login_required
def chat_view(request: HttpRequest, chatroom_name="Lobby"):
...
@login_required
def chatroom_edit_view(request: HttpRequest, chatroom_name):
    chat_group = get_object_or_404(ChatGroup, group_name=chatroom_name)
    ...
    return render(request, "a_rtchat/chatroom_edit.html", context=context)
...
def chatroom_leave_view(request, chatroom_name):
    ...
    if request.method == "POST":
        if request.user == chat_group.admin:
            messages.error(
                request, "You cannot leave this chat because you are the admin"
            )
            return redirect("chatroom", chatroom_name)
        else:
...
def chat_file_upload(request: HttpRequest, chatroom_name):
    chat_group = get_object_or_404(ChatGroup, group_name=chatroom_name)
